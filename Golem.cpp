...
void Golem::M_Set_attacktime(float _time)
{
	m_attackCycle = _time;
	...
	, M_flowingtime()
{
	...
	M_animeintance->M_AddAnimation(L"Resources\\enemy\\Golem\\golem2.png", 1, 1, ANIMESTATUS::IDLE);
	M_animeintance->M_AddAnimation(L"Resources\\enemy\\Golem\\golem2_attack.png", 7, 0.7f, ANIMESTATUS::ATTACK);
	M_animeintance->M_AddAnimation(L"Resources\\enemy\\Golem\\golem2.png", 1, 1, ANIMESTATUS::DIE);
...
void Golem::M_Update()
{
	...
	if (M_GetOnhit() == true)
	{
		...
		{
			//몬스터가 미사일에 맞은 데미지
			SoundManager::GetInstance()->M_PlayEffect(SOUND_LIST::GOBLIN_HIT, SOUND_CHANNEL::SOUND_CHANNEL_MOSTERHIT, 1.f);
	...
	{
		//SoundManager::GetInstance()->M_PlayEffect(SOUND_LIST::GOLEM_ATTACK, SOUND_CHANNEL::SOUND_CHANNEL_MOSTERATTACK, 1);
	...
	if (m_SoundStart)
	{
		...
		if (M_flowingtime>0.45)
		{
			SoundManager::GetInstance()->M_PlayEffect(SOUND_LIST::GOLEM_ATTACK, SOUND_CHANNEL::SOUND_CHANNEL_MOSTERATTACK, 1.f);
			m_SoundStart = false;
			M_flowingtime = 0;
		}
